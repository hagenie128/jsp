...
<script>
	...
	window.onload = () => {
		...
		document.querySelector('#btnSearch').onclick = async (e) => {
			...
			if(list.length === 0){
				tag += `<tr>`;
				tag += `<td colspan="6">조회한 데이터가 없습니다</td>`;
				tag += `</tr>`;
			}else{
				list.forEach(item => {
					//태그 조립하고 화면에 출력 
					...
					tag += `<td>\${item.no}</td>`;
					tag += `<td>\${item.id}</td>`;
					tag += `<td class="password-cell">\${item.passwd}</td>`;
			...
			<tr>
				<td colspan="6">
					<div class="search-box">
						...
						</select> <input type="text" id="search" placeholder="검색어 입력">
			...
			<c:forEach items="${list}" var="member">
				<tr>
					...
					<td>
						...
						<a class="btnupdateView" href="./updateView.do?no=${member.no }">수정</a>
					</td>
				</tr>
			</c:forEach>
